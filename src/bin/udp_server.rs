...
fn main() {
    let socket = UdpSocket::bind("127.0.0.1:3000").expect("Unable to bind the port");

    let mut buffer = [0; 1024];

    loop {
        let socket_new = socket.try_clone().expect("Unable to clone the socket");

        match socket_new.recv_from(&mut buffer) {
            Ok((num_bytes, src_addr)) => thread::spawn(move || {
